...
//
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
vector<unique_ptr<CScene_X::MODEL_LOAD>>  CScene_X::m_pModelLoad;	// モデル情報の読み込み用
...
void CScene_X::Draw(void)
{
	...
	if (m_bShadowMap)
	{
		...
		CShadowmapping::Draw(
			pDevice,	// デバイス情報
			m_pModelLoad[m_nModelId].get(),	// モデル情報
			m_mtxWorld	// マトリックス情報
		);
		// ステンシルバッファを有効にする
...
HRESULT CScene_X::LoadModel(void)
{
	...
	for (int nCntLine = 0; nCntLine < (signed)vsvec_Contens.size(); nCntLine++)
	{
		...
		{
			// 変数宣言
			unique_ptr<MODEL_LOAD> pModelLoad(new MODEL_LOAD);	// モデル情報の読み込み用
			switch (nCntItem)
			{
				// パス情報
			case 0:
