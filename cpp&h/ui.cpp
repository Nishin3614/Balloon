...
CUi::CUi()
{
	...
	m_Uitype = UITYPE_GAMEDIED;	// UIタイプ
	m_nUi = 0;					// UI
	//m_nCount = 0;				// カウント
}
...
{
	// プレゼンの情報がヌルではない場合
	if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents != NULL)
	{
		// プレゼン情報作成
		m_pPresents = C2DPresents::Create_Self(
			(CScene_TWO::OFFSET_TYPE)m_vec_UiLoad[m_Uitype].at(m_nUi).nOffsetType,
			m_vec_UiLoad[m_Uitype].at(m_nUi).pos,
			m_vec_UiLoad[m_Uitype].at(m_nUi).size,
			m_vec_UiLoad[m_Uitype].at(m_nUi).fRot,
			m_vec_UiLoad[m_Uitype].at(m_nUi).col
		);
		// テクスチャー座標設定
		m_pPresents->SetTex(m_vec_UiLoad[m_Uitype].at(m_nUi).tex_first, m_vec_UiLoad[m_Uitype].at(m_nUi).tex_last);
		// テクスチャー設定
		m_pPresents->BindTexture(CTexture_manager::GetTexture(m_vec_UiLoad[m_Uitype].at(m_nUi).nTexType));
		// 強調情報がヌルではない場合
		if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation != NULL)
		{
			// 色情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pCol != NULL)
			{
				// 色の設定
				m_pPresents->SetCooperation(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pCol->changecol,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pCol->nTimeSwitch,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pCol->nTimeChange
				);
			}
			// 拡大率情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pScal != NULL)
			{
				// 拡大率の設定
				m_pPresents->SetCooperation(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pScal->fChangeScal,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pCooperation->pScal->nTimeChange
				);
			}
		}
		// フェードイン情報がヌルではない場合
		if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn != NULL)
		{
			// フェードイン(位置指定)情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadePos != NULL)
			{
				// フェードイン(位置指定)の設定
				m_pPresents->SetFadeIn(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadePos->BeginPos,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadePos->EndPos,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadePos->nMaxfram
				);
			}
			// フェードイン(色指定)情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadeCol != NULL)
			{
				// フェードイン(色指定)の設定
				m_pPresents->SetFadeIn(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadeCol->BeginCol,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadeCol->EndCol,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->pFadeCol->nMaxfram
				);
			}
			// フェードイン強制終了情報設定
			m_pPresents->SetFadeIn_Compulsion(m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->bSkip);
			// フェードイン(スタート時間指定)の設定
			m_pPresents->SetFadeIn_StarAnim(
				m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeIn->FadeStart
			);
			// フェードイン開始
			m_pPresents->Start_FadeIn();
		}
		// フェードアウト情報がヌルではない場合
		if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut != NULL)
		{
			// フェードアウト(位置指定)情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadePos != NULL)
			{
				// フェードアウト(位置指定)の設定
				m_pPresents->SetFadeOut(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadePos->BeginPos,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadePos->EndPos,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadePos->nMaxfram
				);
			}
			// フェードアウト(色指定)情報がヌルではない場合
			if (m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadeCol != NULL)
			{
				// フェードアウト(色指定)の設定
				m_pPresents->SetFadeOut(
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadeCol->BeginCol,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadeCol->EndCol,
					m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->pFadeCol->nMaxfram
				);
			}
			// フェードアウト強制終了情報設定
			m_pPresents->SetFadeOut_Compulsion(m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->bSkip);
			// フェードアウト(スタート時間指定)の設定
			m_pPresents->SetFadeOut_StarAnim(
				m_vec_UiLoad[m_Uitype].at(m_nUi).pPresents->pFadeOut->FadeStart
			);
	...
	else
	{
		...
		// テクスチャー座標設定
		m_pScene_two->SetTex(m_vec_UiLoad[m_Uitype].at(m_nUi).tex_first, m_vec_UiLoad[m_Uitype].at(m_nUi).tex_last);
		// テクスチャー設定
		m_pScene_two->BindTexture(CTexture_manager::GetTexture(m_vec_UiLoad[m_Uitype].at(m_nUi).nTexType));
	}
	...
	{
		// ゲームクリア
	case UITYPE_GAMECLEAR:
		Init_UiType_GameOver();
		break;
		// ゲーム死亡
	case UITYPE_GAMEDIED:
		Init_UiType_GameDied();
		break;
		// ゲームUI
	case UITYPE_GAMEUI:
		...
		break;
		// ポーズUI背景
	case UITYPE_PAUSEUI_BG:

		break;
		// ランキングUI
	case UITYPE_RANKINGUI:

		break;
		// ゲーム設定UI背景
	case UITYPE_SETTINGUI_BG:

		break;
		// タイトルUI名前
	case UITYPE_TITLEUI_NAME:
		Init_UiType_Title_Ui_Name();
		break;
		// チュートリアル_攻撃
	case UITYPE_TUTORIAL_ATTACK:

		break;
		// チュートリアル_終了
	case UITYPE_TUTORIAL_END:

		break;
		// チュートリアル_秘奥義
	case UITYPE_TUTORIAL_TRICK:

		break;
		// チュートリアル_歩く
	case UITYPE_TUTORIAL_WALK:
...
void CUi::Update(void)
{
	...
	case UITYPE_RANKINGUI:
		...
		// タイトルUI名前
	case UITYPE_TITLEUI_NAME:
		Update_UiType_Title_Ui_Name();
		break;
		// チュートリアル_攻撃
	case UITYPE_TUTORIAL_ATTACK:
		Update_UiType_Tutorial_Attack();
		break;
		// チュートリアル_終了
	case UITYPE_TUTORIAL_END:
		Update_UiType_Tutorial_End();
		break;
		// チュートリアル_秘奥義
	case UITYPE_TUTORIAL_TRICK:
		Update_UiType_Tutorial_Trick();
		break;
		// チュートリアル_歩く
	case UITYPE_TUTORIAL_WALK:
		Update_UiType_Tutorial_Walk();
		break;
		// チュートリアル_合格
	case UITYPE_TUTORIAL_CLEAR:
		Update_UiType_Tutorial_Clear();
		break;
	default:
		break;
	}
...
// シーン管理しているものに限る
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::End_Ui(UITYPE const & uitype)
{
...
// タイトルUIの名前の初期化
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Init_UiType_Title_Ui_Name(void)
{
}
...
// タイトル選択の初期化
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Init_UiType_Title_Select(void)
{
}
...
// ゲームスタート時の初期化
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Init_UiType_GameStart(void)
{
}
...
// ゲームオーバー時の初期化
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Init_UiType_GameOver(void)
{
}
...
// ゲーム死亡時の初期化
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Init_UiType_GameDied(void)
{
}
...
// ゲームオーバーの更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_GameOver(void)
{
...
// ゲームスタートの更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_GameStart(void)
{
}
...
// ゲーム死亡の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_GameDied(void)
{
...
// タイトルUI名前の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Title_Ui_Name(void)
{
}
...
// タイトル選択更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Title_Select(void)
{
}
...
// タイトルチュートリアルスキップ更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Title_TutorialSkip(void)
{
...
// チュートリアル歩くの更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Tutorial_Walk(void)
{
}
...
// チュートリアル攻撃の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Tutorial_Attack(void)
{
}
...
// チュートリアル秘奥義の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Tutorial_Trick(void)
{
}
...
// チュートリアル終了の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Tutorial_End(void)
{
}
...
// チュートリアル合格の更新
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CUi::Update_UiType_Tutorial_Clear(void)
{
}